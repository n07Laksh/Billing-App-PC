  ...
  useEffect(() => {
    ...
    retrieveItemsForCurrentDate(saleDB.saleItems).then(items => {
      if (items.length > 0) {
        ...
        const totalSaleAmount = items.reduce((sum, item) => sum + item.amount, 0);
        setTodaySale(totalSaleAmount);
      } else {
        setTodaySale("00"); // Set zero if no items found for today's date
      }
    });
    ...
    // Example usage for calculating the total sale
    retrieveAllItems(saleDB.saleItems).then(items => {
      if (items.length > 0) {
        ...
        setTotalySale(0); // Set zero if no items found
      }
    });

    // Example usage for calculating the total purchase
    retrieveAllItems(purchaseDB.purchaseData).then(items => {
      if (items.length > 0) {
        // Calculate total amount for purchases
        const totalPurchaseAmount = items.reduce((sum, item) => sum + item.amount, 0);
        setTotalPurchase(totalPurchaseAmount);
      } else {
        setTotalPurchase(0); // Set zero if no items found
      }
    });
...
retrievePreviousMonthItems(saleDB.saleItems).then(items => {
  if (items.length > 0) {
    // Calculate total amount for sales
    const totalPreviousMonthSale = items.reduce((sum, item) => sum + item.amount, 0);
    setPreviousMonthSale(totalPreviousMonthSale);
  } else {
    setPreviousMonthSale(0); // Set zero if no items found
  }
});
...
retrievePreviousMonthItems(purchaseDB.purchaseData).then(items => {
  if (items.length > 0) {
    // Calculate total amount for purchases
    const totalPreviousMonthPurchase = items.reduce((sum, item) => sum + item.amount, 0);
    setPreviousMonthPurchase(totalPreviousMonthPurchase);
  } else {
    setPreviousMonthPurchase(0); // Set zero if no items found
  }
});
          ...
          <div>
            <ul className="total-ul">
              <li className="total-ul-li">
                ...
                <span className="text-primary fs-6 fw-bold"> {previousSale} </span>
              </li>
              <li className="total-ul-li">
                <span>Previous Month total Purchase</span>
                <span className="text-primary fs-6 fw-bold"> {previousMonthPurchase} </span></li>
            </ul>
          </div>
        </div>
